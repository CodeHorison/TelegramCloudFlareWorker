...
function normalizeDst(dst) {
  if (typeof dst !== "string") return null;
...
export default {
  async fetch(request) {
    ...
    const server = pair[1];
    server.accept();
    server.binaryType = "arraybuffer"; // treat binary frames as ArrayBuffer (no Blob overhead)
    ...
    function cleanup(code, reason) {
      ...
      // Close TCP writer first (this releases its lock), then release reader
      if (tcpWriter) {
        try {
      ...
      // Close the server-side WebSocket if still open
